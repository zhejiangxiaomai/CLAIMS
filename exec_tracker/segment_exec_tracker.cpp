...
class SegmentExecTrackerActor : public event_based_actor {
 public:
  SegmentExecTrackerActor(actor_config& cfg,
                          SegmentExecTracker* segment_exec_tracker)
    ...
    return {
          ...
          if (seg_exec_tracker_->node_segment_id_to_status_.size() > 0) {
            ...
            for (; it != seg_exec_tracker_->node_segment_id_to_status_.end();) {
              ...
              if (it->second->stop_report_) {
                ...
                send(this, ReportSAtom::value, (*it->second));  // bug han
                ++it;
              }
            }
          }
        ...
        [&](ReportSAtom, SegmentExecStatus& seg_exec_status) {
          ...
          if (seg_exec_status.stop_report_ == true) {
            ...
            request(*coor_actor_, std::chrono::seconds(kTimeout),
...
SegmentExecTracker::SegmentExecTracker() {
  ...
  cfg1.load<io::middleman>();
  actor_system system{cfg1};
  auto segment_exec_tracker_actor_ =
      system.spawn<SegmentExecTrackerActor>(this);
  //由于暂时没有句柄 暂时定死端口和ip。
  auto expected = system.middleman().publish(segment_exec_tracker_actor_, 20000,
                                             "127.0.0.1", true);
  if (!expected) {
...
//                      seg_exec_status->node_segment_id_, exec_status,
//                      exec_info)
//                .then(
//
//                    [=](OkAtom) {
...
//                    >
//                    // TryReportTimes, then the network may be error, so
//                    cancel
//                    // it
